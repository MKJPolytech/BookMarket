...
import kr.ac.kopo.mkj.bookmarket.exception.bookIdException;
import kr.ac.kopo.mkj.bookmarket.exception.categoryException;
...
public class BookController {
    ...
    @PostMapping("/add")
    public String requestSubmitNewBook(@Valid @ModelAttribute("book") Book book,
                                       BindingResult bindingResult) {
        if (bindingResult.hasErrors()) {
            return "addbook";
        }
        MultipartFile multipartFile = book.getBookImage();
        if (multipartFile != null && !multipartFile.isEmpty()) {           // ✅ 안전 체크
            String saveName = multipartFile.getOriginalFilename();
            File saveFile = new File(fileDir, saveName);                    // ✅ 경로 결합 안전
            try {
                multipartFile.transferTo(saveFile);                         // ✅ 올바른 대상
            } catch (IOException e){
    ...
    @GetMapping("/download")
    public void downloadBookImage(@RequestParam("fileName") String paramKey,
                                  HttpServletResponse response) throws IOException {
        File imageFile = new File(fileDir, paramKey);                       // ✅ 구분자 보장
        if (!imageFile.exists()) {
...
//        binder.setValidator(UnitsInStockValidator);
        booksValidator.setSpringValidators(Set.of(unitsInStockValidator));
